...
Feature: Get tasks
  ...
  Scenario: Admin views user's tasks
    When the client submits a GET request to /users/2/tasks with the 1st user's credentials
    Then the JSON response should include all the 2nd user's tasks
    And the response should return status 200
  ...
  Scenario: User views one of their own tasks
    When the client submits a GET request to /tasks/6 with the 2nd user's credentials
    Then the JSON response should include the 6th task
    And the response should return status 200

  Scenario: Admin views one of a user's tasks
    When the client submits a GET request to /tasks/6 with the 1st user's credentials
    Then the JSON response should include the 6th task
    And the response should return status 200

  Scenario: User attempts to view someone else's task
    When the client submits a GET request to /tasks/1 with the 2nd user's credentials
    Then the response should not include any data
    And the response should indicate the request was unauthorized

  Scenario: User attempts to view task without authenticating
    When the client submits a GET request to /tasks/1 with no credentials
    Then the response should not include any data
    And the response should indicate the request was unauthorized
